...
@endforeach
                    ...
                    <tbody>
                        <form action="{{ route('student_attendance_store') }}" method="GET">
...
@for ($i = 1; $i <= $todayDate->day; ++$i)
    @php
        ...
        $check_attd = \App\Models\Student_attendance::where('student_id', $student->id)
            ->where('attendance_date', $date_picker)
            ->first();
 @endphp
        {{-- $check_leave = \App\Models\Leave::where('student_id', $student->id)
            ->where('leave_date', $date_picker)
            ->first(); --}}
     @php
    ...
    <td>
        <div class="form-check form-check-inline">
            <input class="form-check-input"
                name="attd[{{ $date_picker }}][{{ $student->id }}]"
                type="checkbox"
                @if ($check_attd) checked @endif
        ...
        <div class="form-check form-check-inline">
            <input class="form-check-input"
                name="leave[{{ $date_picker }}][{{ $student->id }}]"
                type="checkbox"
                {{-- @if ($check_leave) checked @endif --}}
                @if (!$is_editable) disabled @endif
                value="1">
